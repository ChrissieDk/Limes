...
import {
  ...
  isFirebaseAuthError,
} from "../utils/firebaseAuthErrorMessage";
...
export default function SignUp() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    setSubmitting(true);
    try {
      const cred = await firebaseAuthService.signUpWithEmailPassword({
        email: values.email,
        password: values.password,
      });
      const displayName = cred.user.displayName?.trim() || "";
      const [firstNameFromDisplay, ...rest] = displayName
        .split(" ")
        .filter(Boolean);
  ...
  const formContent = showVerificationMessage ? (
    <div className="grid gap-4">
      <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
        <h3 className="font-grotesque text-green-400 font-semibold mb-2 text-center">
          Check your email!
        </h3>
    ...
    <>
      ...
      <div className="hidden lg:block">
        <AuthLayout
          ...
          subheading={
            <>
              Join the community where your mobile subscription actually makes
              sense for you.
            </>
          }
          side={
      ...
      {/* Mobile */}
      <MobilePage title="Sign up" backTo="/">
        <div className="flex flex-col justify-center min-h-[calc(100vh-180px)] px-6">
          <div className="mb-8">
            ...
            <p className="font-manrope text-neutral-400 text-sm mt-2">
              Join the network built different
            </p>
          </div>
          <div className="grid gap-4">
            <TextField
            ...
            <div className="font-manrope pt-2 text-sm text-neutral-500 text-center">
              Already have an account?{" "}
              <Link to="/signin" className="text-[#ABFF63]">
                Sign in
              </Link>
            </div>
          </div>
