...
import {
  getServiceDisplayValue,
  convertRandsToServiceValue,
  getDefaultExpiryDate,
  toCents,
} from "../../payment/utils/dynamicPricing";
        ...
        >
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="font-grotesque font-bold text-neutral-900 text-base">
                {product.name}
              </div>
              <div className="font-manrope text-sm text-neutral-600 mt-1.5 line-clamp-1">
                {product.description}
              </div>
            </div>
            <div className="text-right flex-shrink-0">
              <div
                className={`inline-block px-3 py-1.5 rounded-lg ${priceBgColor}`}
              >
                <div className="font-bold text-lg text-neutral-900">
                  R{product.price.toFixed(2)}
                </div>
              </div>
              <div className="font-manrope text-xs text-neutral-500 font-medium mt-1">
                once-off
              </div>
            </div>
          </div>
  ...
  return (
    <div className="flex items-center justify-center gap-5">
      ...
      <div className="flex items-center justify-center gap-1">
        <span className="font-grotesque font-extrabold text-6xl tracking-tight text-neutral-900">
          R
        </span>
        <input
          type="text"
      ...
      <div className="rounded-2xl bg-neutral-50 p-5 space-y-3">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-semibold text-neutral-900">
            {label === "Subscription" ? amount : "Airtime"}
          </span>
        </div>
        <div className="border-t border-neutral-200 pt-3 flex items-center justify-between">
          <span className="font-grotesque font-bold text-neutral-900 text-base">
            Total
          </span>
          <span className="font-grotesque font-bold text-3xl text-neutral-900">
            {amount}
          </span>
        </div>
      </div>
...
export default function TopUpModal({
  ...
  const finalizeBundlePurchase = async (reference: string) => {
    ...
    if (orderResponse.orderId) {
      ...
      log.info("topup_bundle_order_created", {
        reference,
        order_id: orderResponse.orderId,
      });

      // Process the pending order so the bundle is provisioned immediately.
      // Without this, the order sits pending and balance API won't reflect it.
      try {
        await subscriptionService.processPendingOrders(selectedPhoneNumber);
        log.info("topup_bundle_order_processed", { reference });
      } catch (processErr) {
        // Processing may fail if the backend needs more time, but the order
        // exists and will be processed by the activation flow or a retry.
        log.warn("topup_bundle_order_process_retry_needed", {
          reference,
          error: getAxiosErrorMessage(processErr, "unknown"),
        });
      }
    } else if (!orderResponse.message) {
      log.error("topup_bundle_order_failed", {
        reference,
        response: JSON.stringify(orderResponse),
      });
      throw new Error(
        "Order creation failed - no orderId or message in response",
      );
    }
  ...
  const finalizeDynamicServicePurchase = async (reference: string) => {
    ...
    const sv = convertRandsToServiceValue(
      kind.toUpperCase() as ServiceType,
      price,
      "prepaid",
    );
    if (sv === null) {
    ...
    const dc = (
      kind.toUpperCase() === "AIRTIME" ? "GPA_CREDIT" : kind.toUpperCase()
    ) as "DATA" | "VOICE" | "SMS" | "WHATSAPP" | "GPA_CREDIT";
    const servicesResponse = await subscriptionService.createDynamicServices(
      selectedPhoneNumber,
      {
        services: [
          { value: sv, definitionCode: dc, expiryDate: getDefaultExpiryDate() },
        ],
      },
    );
    ...
    if (serviceIds.length === 0) {
      log.error("topup_dynamic_no_services_created", {
        reference,
        response: JSON.stringify(servicesResponse.results),
      });
    ...
    log.info("topup_dynamic_services_linked", {
      reference,
  ...
  const handlePurchaseBundle = async () => {
    if (!selectedProduct || !selectedPhoneNumber) {
      ...
      log.warn("topup_bundle_init_failed", {
        reason: "missing_product_or_phone",
      });
      return;
    }
    ...
    if (!selectedProduct.price) {
      ...
      log.error("topup_bundle_init_failed", {
    ...
    try {
      const status =
        await subscriptionService.checkSimActive(selectedPhoneNumber);
      if (!status.isActive) {
        setPaymentError(
          "This SIM is not active. Please wait for activation to complete before topping up.",
        );
      ...
      log.warn("topup_bundle_sim_status_check_failed", {
        msisdn: selectedPhoneNumber,
        error: getAxiosErrorMessage(err, "unknown"),
      });
    }
    ...
    try {
      ...
      if (selectedPaymentMethodId) {
        ...
        if (!chargeResponse.success || !chargeResponse.transaction) {
          log.error("topup_bundle_charge_failed", {
            error: chargeResponse.error || "unknown",
          });
          setPaymentError(
            chargeResponse.error || "Failed to charge saved card",
          );
          return;
        }
      ...
      if (!initResponse.success || !initResponse.data) {
        log.error("topup_bundle_init_failed", {
          error: initResponse.error || "unknown",
        });
        setPaymentError(initResponse.error || "Failed to initialize payment");
        return;
      }

      log.info("topup_bundle_initialized", {
      ...
      popup.resumeTransaction(initResponse.data.access_code, {
        onSuccess: async (transaction: Record<string, unknown>) => {
          try {
            const reference = String(
              transaction.reference || initResponse.data?.reference || "",
            );
            ...
            if (!verificationResponse.success) {
              log.error("topup_bundle_verification_failed", {
                reference,
                error: verificationResponse.error || "unknown",
              });
              throw new Error(
            ...
            const errMsg = getAxiosErrorMessage(
              err,
        ...
        onCancel: () => {
          log.warn("topup_bundle_cancelled_by_user", {
            product_id: selectedProduct.id,
          });
          setPaymentError(null);
        },
  ...
  const handlePurchaseDynamicService = async () => {
    if (!selectedPhoneNumber || kind === "bundles") {
      ...
      log.warn("topup_dynamic_init_failed", {
        reason: "missing_phone_or_wrong_kind",
        kind,
      });
      return;
    }
    ...
    try {
      ...
      // If the check itself fails, log and allow the purchase to proceed.
      // The backend is the ultimate gatekeeper.
      log.warn("topup_dynamic_sim_status_check_failed", {
        msisdn: selectedPhoneNumber,
        error: getAxiosErrorMessage(err, "unknown"),
      });
    }
    ...
    try {
      ...
      const serviceValue = convertRandsToServiceValue(
        serviceType,
        price,
        "prepaid",
      );
      ...
      const definitionCode =
        serviceType === "AIRTIME" ? "GPA_CREDIT" : serviceType;
      ...
      if (selectedPaymentMethodId) {
        ...
        if (!chargeResponse.success || !chargeResponse.transaction) {
          log.error("topup_dynamic_charge_failed", {
            error: chargeResponse.error || "unknown",
          });
          setPaymentError(
            chargeResponse.error || "Failed to charge saved card",
      ...
      const payload = {
        ...
        services: [
          {
            value: serviceValue,
            definitionCode: definitionCode as
              | "DATA"
              | "VOICE"
              | "SMS"
              | "WHATSAPP"
      ...
      const initResponse =
        await dynamicServicesPaymentService.initializePayment(payload);

      if (!initResponse.success || !initResponse.data) {
        log.error("topup_dynamic_init_failed", {
          error: initResponse.error || "unknown",
        });
        setPaymentError(initResponse.error || "Failed to initialize payment");
        return;
      ...
      log.info("topup_dynamic_initialized", {
        reference: initResponse.data.reference,
      });
      ...
      popup.resumeTransaction(initResponse.data.access_code, {
        onSuccess: async (transaction: Record<string, unknown>) => {
          try {
            ...
            if (!verificationResponse.success) {
              log.error("topup_dynamic_verification_failed", {
                reference,
                error: verificationResponse.error || "unknown",
              });
              throw new Error(
        ...
        onCancel: () => {
          log.warn("topup_dynamic_cancelled_by_user", {
            msisdn: selectedPhoneNumber,
            kind,
            price,
          });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3">
      ...
      <div className="relative w-full max-w-lg sm:max-w-xl mx-0 sm:mx-4 rounded-3xl bg-white text-neutral-900 shadow-2xl animate-in fade-in zoom-in duration-200 max-h-[82vh] sm:max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-neutral-200 sticky top-0 bg-white z-10 rounded-t-3xl">
          <div>
            <div className="font-grotesque font-bold text-xl text-neutral-900">
              Top-up
            </div>
            <div className="font-manrope text-sm text-neutral-500 mt-0.5">
              Enter the details below to top-up
            </div>
          </div>
          <button
            aria-label="Close"
            className="size-10 grid place-items-center rounded-full text-neutral-400 hover:bg-neutral-100 hover:text-neutral-600 text-2xl transition-colors"
            onClick={onClose}
          >
            ×
          </button>
          ...
          <div className="px-6 pt-5 pb-6 space-y-5">
              ...
              <>
                ...
                <div className="flex items-center justify-center">
                  <div className="inline-flex items-center gap-2 rounded-full bg-[#ABFF63]/20 px-4 py-2.5">
                    <span className="font-manrope text-sm text-neutral-700">
                      You'll get:
                    </span>
            ...
            {kind === "bundles" && !selectedCategory && (
              <div className="space-y-4">
                <h3 className="font-grotesque text-neutral-900 font-semibold text-base">
                  Choose Bundle Type
                </h3>
                ...
                {loading && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        key={i}
                        className="animate-pulse rounded-2xl border border-neutral-200 p-4 h-28 bg-neutral-50"
                      />
                    ))}
                  </div>
                )}
                ...
                {loading ? (
                  <div className="space-y-3">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="animate-pulse rounded-2xl border border-neutral-200 p-5 h-24 bg-neutral-50"
                      />
                    ))}
                  </div>
                ) : products.length > 0 ? (
            ...
            {(selectedProduct || kind !== "bundles") && (
              <div className="space-y-2">
                <div className="font-grotesque text-neutral-700 text-sm font-semibold">
                  Phone number to top-up
                </div>
                <div className="relative">
                  <button
                    className="w-full flex items-center gap-3 rounded-xl ring-1 ring-neutral-300 px-4 py-3 bg-white text-left hover:ring-neutral-400 transition-all"
                  ...
                  >
                    <img
                      ...
                      className="h-6 w-6"
                    />
                    <span className="flex-1 text-neutral-900 font-medium">
                      {selectedPhoneNumber || "Select a SIM"}
                    </span>
                    <span
                      className={`text-neutral-400 transition-transform text-xl leading-none ${isPhoneMenuOpen ? "rotate-180" : ""}`}
                    >
                      ▾
                    </span>
                  </button>
                    ...
                    <div className="absolute left-0 right-0 mt-2 z-10 rounded-xl bg-white ring-1 ring-neutral-200 shadow-lg overflow-hidden max-h-48 overflow-y-auto">
                      {(phoneNumbers && phoneNumbers.length > 0
                        ...
                        .map((num) => (
                          ...
                          >
                            <span className="font-manrope inline-flex items-center justify-center size-6 rounded bg-[#ABFF63] text-neutral-900 text-xs font-bold">
                              SIM
                            </span>
            ...
            {paymentSuccess && (
              <div className="rounded-xl bg-green-50 border-2 border-green-200 p-4 flex items-center gap-3">
                ...
                <p className="font-manrope text-sm font-medium text-green-700">
