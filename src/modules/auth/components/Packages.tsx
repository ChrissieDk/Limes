...
const Packages = memo(function Packages() {
  ...
  const cards: Card[] = [
    ...
    },
    {
      ...
      textClass: "text-neutral-900",
      bullets: [
        "Unlimited voice minutes + 10GB data",
        "10GB data",
        "Unlimited WhatsApp text",
      ],
      colSpanClass: "lg:col-span-3",
    ...
    <section id="packages" className="relative bg-neutral-900">
      <div className="mx-auto max-w-6xl px-6 pt-14 pb-24">
        <div className="flex items-center justify-center font-grotesque font-semibold text-neutral-400 text-[24px] sm:text-[30px] md:text-[36px] leading-[1.05]">
          <span className="w-2.5 h-2.5 rounded-full bg-pink-300 mr-3 translate-y-[1px]" />{" "}
          Our Subscriptions
        </div>
            ...
            {cards.map((card) => (
              ...
              >
                ...
                <ul className="mt-2.5 space-y-2 text-[14px] leading-snug font-manrope">
                  {card.bullets.map((b) => {
                    const iconSrc =
                      card.key === "limes-unlimited" || card.key === "limes-one"
                        ? bulletSrcYellow
                        : bulletSrc;
                    return (
                      <li key={b} className="flex items-start gap-2">
                        <img
                          src={iconSrc}
                          alt=""
                          className="h-4 w-5 mt-[2px] object-contain"
                        />
                        <span>{b}</span>
                      </li>
                    );
