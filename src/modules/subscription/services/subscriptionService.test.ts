...
describe('subscriptionService', () => {
  ...
  describe('createOrder', () => {
    it('posts to /order/create with timeout', async () => {
      const payload = { products: [{ id: '123', amount: 100 }], msisdn: '27612345678' }
      const response = { orderId: 'ord-1' }
      mockPost.mockResolvedValue({ data: response })

      const result = await subscriptionService.createOrder(payload)

      expect(mockPost).toHaveBeenCalledWith('/order/create', payload, { timeout: API_TIMEOUT_MS })
      expect(result).toEqual(response)
    })
  })
  ...
  describe('processPendingOrders', () => {
    it('processes pending orders for MSISDN', async () => {
      ...
      const result = await subscriptionService.processPendingOrders('27612345678')

      expect(mockPost).toHaveBeenCalledWith('/order/pending/27612345678/process', {}, { timeout: API_TIMEOUT_MS })
      expect(result).toEqual(response)
    })
  })
  ...
  describe('processPendingDynamicServices', () => {
    it('processes pending dynamic services for MSISDN', async () => {
      ...
      const result = await subscriptionService.processPendingDynamicServices('27612345678')

      expect(mockPost).toHaveBeenCalledWith('/subscriber/27612345678/service/dynamic/pending/process', {}, { timeout: API_TIMEOUT_MS })
      expect(result).toEqual(response)
    })
  })
  ...
  describe('storePendingOrder', () => {
    it('stores pending order', async () => {
      const payload = { msisdn: '27612345678', productId: '123', productAmount: 100, paymentReference: 'ref-1' }
      const response = { success: true, message: 'Stored' }
      mockPost.mockResolvedValue({ data: response })

      const result = await subscriptionService.storePendingOrder(payload)

      expect(mockPost).toHaveBeenCalledWith('/order/pending', payload, { timeout: API_TIMEOUT_MS })
      expect(result).toEqual(response)
    })
  })
  ...
  describe('storePendingDynamicService', () => {
    it('stores pending dynamic service', async () => {
      ...
      const result = await subscriptionService.storePendingDynamicService('27612345678', payload)

      expect(mockPost).toHaveBeenCalledWith('/subscriber/27612345678/service/dynamic/pending', payload, { timeout: API_TIMEOUT_MS })
      expect(result).toEqual(response)
    })
  })
})
