...
import LimesGreen from "../assets/logos/Limes Green Logo.png";
...
const NavBar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-transparent">
      <nav className="mx-auto max-w-7xl sm:px-6 lg:px-8 flex items-center justify-between bg-white rounded-xl px-6 py-4 shadow-lg">
        {/* Logo */}
        ...
        <div className="hidden md:flex items-center space-x-8">
          {navItems.map((item, index) => (
            <Link
              key={index}
              to={item.path}
              className="text-gray-800 transition duration-300 relative group pb-2 pt-2 font-semibold"
            >
              {item.text}
              <span
      ...
      {/* Mobile Dropdown */}
      {isMenuOpen && (
        <div className="md:hidden bg-white mt-2 mx-4 rounded-lg p-4 shadow-lg">
